...
const FIELD_COLUMNS = [
  ...
  ["bidNtceName", "bid_ntce_name"],
  ["noticeKindName", "notice_kind_name"],
  ["registrationTypeName", "registration_type_name"],
  ["referenceNo", "reference_no"],
  ["noticeInstitutionCode", "notice_institution_code"],
  ["noticeInstitutionName", "notice_institution_name"],
  ["demandInstitutionCode", "demand_institution_code"],
  ["demandInstitutionName", "demand_institution_name"],
  ["contractMethodName", "contract_method_name"],
  ["bidMethodName", "bid_method_name"],
  ["awardMethodCode", "award_method_code"],
  ["awardMethodName", "award_method_name"],
  ["noticePostedRaw", "notice_posted_raw"],
  ["noticePostedLocal", "notice_posted_local"],
  ["bidBeginRaw", "bid_begin_raw"],
  ["bidBeginLocal", "bid_begin_local"],
  ["bidCloseRaw", "bid_close_raw"],
  ["bidCloseLocal", "bid_close_local"],
  ["openingRaw", "opening_raw"],
  ["openingLocal", "opening_local"],
  ["registeredRaw", "registered_raw"],
  ["registeredLocal", "registered_local"],
  ["changedRaw", "changed_raw"],
  ["changedLocal", "changed_local"],
  ["detailedProductClassNo", "detailed_product_class_no"],
  ["detailedProductClassName", "detailed_product_class_name"],
  ["productQuantity", "product_quantity"],
  ["productUnit", "product_unit"],
  ["productUnitPrice", "product_unit_price"],
  ["productSpecification", "product_specification"],
  ["purchaseProductListRaw", "purchase_product_list_raw"],
  ["allocatedBudgetAmount", "allocated_budget_amount"],
  ["estimatedPrice", "estimated_price"],
  ["vatAmount", "vat_amount"],
  ["industryVatAmount", "industry_vat_amount"],
  ["internationalBidYn", "international_bid_yn"],
  ["reNoticeYn", "re_notice_yn"],
  ["rebidPermittedYn", "rebid_permitted_yn"],
  ["manufactureYn", "manufacture_yn"],
  ["designatedCompetitionYn", "designated_competition_yn"],
  ["productClassLimitYn", "product_class_limit_yn"],
  ["noticeUrl", "notice_url"],
  ["noticeDetailUrl", "notice_detail_url"],
  ["standardNoticeDocumentUrl", "standard_notice_document_url"]
];
function numberId(value, name) {
  if (typeof value !== "number" && typeof value !== "bigint") throw new Error(`Missing ${name}`);
  return Number(value);
}
function normalizeBidNoticeRawItem(database, rawItemId, normalizedAt) {
  const rawRow = database.prepare("SELECT service, operation, canonical_json FROM api_raw_item WHERE raw_item_id = ?").get(rawItemId);
  ...
  const raw = JSON.parse(rawRow.canonical_json);
  ...
  try {
    const existing = database.prepare(`SELECT bid_notice_id, semantic_row_hash, semantic_state_json, source_raw_item_id
      FROM bid_notice WHERE bid_ntce_no = ? AND bid_ntce_ord = ?`).get(normalized.candidate.bidNtceNo, normalized.candidate.bidNtceOrd);
    if (!existing) {
      const columns = FIELD_COLUMNS.map(([, column]) => column);
      const values2 = FIELD_COLUMNS.map(([field]) => normalized.candidate[field]);
      const result = database.prepare(`INSERT INTO bid_notice
        ...
        normalized.semanticRowHash,
        normalized.semanticStateJson,
        warningsJson,
        normalizedAt,
        normalizedAt
      );
    ...
    if (existing.semantic_row_hash === normalized.semanticRowHash) {
      database.prepare(`UPDATE bid_notice SET source_raw_item_id = ?, source_operation = ?,
        parse_warnings_json = ?, last_normalized_at = ? WHERE bid_notice_id = ?`).run(rawItemId, BID_NOTICE_OPERATION, warningsJson, normalizedAt, bidNoticeId);
      database.exec("COMMIT");
    ...
    database.prepare(`INSERT INTO bid_notice_revision
      ...
      bidNoticeId,
      normalizedAt,
      existing.semantic_row_hash,
      normalized.semanticRowHash,
      existing.source_raw_item_id,
      rawItemId,
      existing.semantic_state_json,
      normalized.semanticStateJson
    );
